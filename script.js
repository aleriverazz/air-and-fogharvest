/* ═══════════════════════════════════════════════════════════════════════
   FogHarvest v5 · script.js
   ─────────────────────────────────────────────────────────────────────
   Fixes vs v4:
   • 429 Archive errors → serialised queue (1.2s gap) + retry backoff
   • Slow load → reduced grid (4×3), forecast-first, loader shows fast
   • Opacity popovers → click wind/hum button → slider appears above map
   • Terrain → always adds mt-dem source + setTerrain on map load
   • Cache → URL-keyed Map, cleared on alt/month change
═══════════════════════════════════════════════════════════════════════ */
...
const CFG = {
  ...
  COAST_RADIUS_KM: 30,
  GRID: { COLS: 14, ROWS: 10 },   // smaller grid = fewer API calls on load

  API_DELAY_MS:  1200,   // minimum ms between archive requests
  API_RETRY_MS:  4000,   // wait before retrying a 429
  API_MAX_RETRY: 2,
...
function initMap() {
  ...
  S.map.on('load', () => {
    /* Always add terrain DEM */
    if (!S.map.getSource('mt-dem')) {
      S.map.addSource('mt-dem', {
        type:'raster-dem',
    ...
    if (!S.map.getLayer('sky')) {
      S.map.addLayer({
        id:'sky', type:'sky',
        paint:{ 'sky-type':'atmosphere','sky-atmosphere-sun':[0,88],'sky-atmosphere-sun-intensity':10 },
      });
    }
...
async function geocodeSearch(query) {
  ...
  try {
    ...
    dropdown.innerHTML=`<div class="search-no-results">Error: ${esc(err.message)}</div>`;
  }
}
...
/* ══════════════════════════════════════════════════════
   5. RATE-LIMITED API — queue + retry + cache
══════════════════════════════════════════════════════ */
function apiRequest(url, isArchive=false) {
  ...
  /* Serialise archive calls with a gap to avoid 429 */
...
async function fetchWithRetry(url, attempt=0) {
  ...
  if(res.status===429){
    if(attempt<CFG.API_MAX_RETRY){
      console.warn(`[FH] 429 — retry ${attempt+1} in ${CFG.API_RETRY_MS*(attempt+1)}ms`);
...
async function fetchWindField() {
  ...
  const month=S.selectedMonth, alt=document.getElementById('selAlt').value;
  const b=map.getBounds(), sw=b.getSouthWest(), ne=b.getNorthEast();

  /* 4×3 sparse sample grid — 12 calls max instead of 20 */
  ...
  try {
    /* Forecast calls run in parallel — archive calls are serialised via apiRequest */
...
/* Sequential mapper — used for archive to avoid stampede */
async function seqMap(arr, fn) {
  const out=[];
  for(const item of arr) out.push(await fn(item));
  return out;
}
...
async function fetchPointData(lat, lon, altMode, month) {
  ...
  if (!isArchive) {
    ...
    url.searchParams.set('hourly',          'relative_humidity_2m,wind_speed_10m,wind_direction_10m,wind_speed_80m,wind_direction_80m,wind_speed_120m,wind_direction_120m,temperature_2m,cloud_cover,precipitation');
    url.searchParams.set('wind_speed_unit', 'ms');
    url.searchParams.set('timezone',        'auto');
    data = await apiRequest(url.toString(), true);
  }

  const c=data.current||{};
  const hAvg=k=>{const a=data.hourly?.[k];if(!a?.length)return null;const v=a.filter(x=>x!=null);return v.length?v.reduce((s,x)=>s+x,0)/v.length:null;};

  const humidity=c['relative_humidity_2m']??hAvg('relative_humidity_2m')??72;
  const temp    =c['temperature_2m']??hAvg('temperature_2m')??15;
  const cloud   =c['cloud_cover']??hAvg('cloud_cover')??0;
  const precip  =c['precipitation']??hAvg('precipitation')??0;

  let speed,dir;
  if(altMode==='10m'){speed=c['wind_speed_10m']??hAvg('wind_speed_10m')??3;dir=c['wind_direction_10m']??hAvg('wind_direction_10m')??270;}
  else if(altMode==='80m'){speed=hAvg('wind_speed_80m')??c['wind_speed_10m']??3;dir=hAvg('wind_direction_80m')??c['wind_direction_10m']??270;}
  else{speed=hAvg('wind_speed_120m')??c['wind_speed_10m']??3;dir=hAvg('wind_direction_120m')??c['wind_direction_10m']??270;}

  return{speed:speed??3,dir:dir??270,humidity,temp,cloud,precip};
}
...
function sampleUV(px,py){
  ...
  return{u:lerp(lerp(wf.uv[ix(r0,c0)],wf.uv[ix(r0,c1)],tc),lerp(wf.uv[ix(r1,c0)],wf.uv[ix(r1,c1)],tc),tr),
...
function startWindParticles(){
  ...
  function frame(){
    ...
    for(const p of S.windParticles){
      ...
      ctx.strokeStyle=windColor(speed);ctx.lineWidth=CFG.WIND.LINE_WIDTH;
      ctx.globalAlpha=base*S.windOpacity;ctx.stroke();
    }
...
/* ══════════════════════════════════════════════════════
   9. OPACITY POPOVERS  ← NEW
   ──────────────────────────────────────────────────────
   Each layer button shows a small popover with:
...
function initOpacityPopovers() {
  function makePopover(id, labelText, layerKey, opacityKey, onOpacityChange, onToggle) {
    ...
    pop.id = id;
    pop.className = 'op-popover hidden';
    pop.innerHTML = `
      <div class="op-header">
        <span class="op-title">${labelText}</span>
        <button class="op-toggle ${S[layerKey]?'on':''}" data-key="${layerKey}">
          ${S[layerKey]?'VISIBLE':'OCULTO'}
        </button>
      </div>
      <div class="op-row">
    ...
    pop.querySelector('.op-slider').addEventListener('input', function() {
      pop.querySelector('.op-pct').textContent = this.value + '%';
      S[opacityKey] = this.value / 100;
      onOpacityChange();
    });
    ...
    pop.querySelector('.op-toggle').addEventListener('click', function(e) {
      e.stopPropagation();
      S[layerKey] = !S[layerKey];
      this.textContent = S[layerKey] ? 'VISIBLE' : 'OCULTO';
      this.classList.toggle('on', S[layerKey]);
      onToggle(S[layerKey]);
    });
  ...
  const windPop = makePopover(
    'windOpPop', 'CAPA DE VIENTO', 'layerWind', 'windOpacity',
    () => { /* opacity already read live by frame loop */ },
    (on) => {
      document.getElementById('btnWind').classList.toggle('active', on);
      if (on) startWindParticles();
      else { cancelAnimationFrame(S.windRAF); S.windCtx.clearRect(0,0,S.windCanvas.width,S.windCanvas.height); }
    }
  );

  const humPop = makePopover(
    'humOpPop', 'CAPA DE HUMEDAD', 'layerHum', 'humOpacity',
    () => { if(S.humField) drawHumidityCanvas(); },
    (on) => {
      document.getElementById('btnHum').classList.toggle('active', on);
      if (on) { if(S.humField) drawHumidityCanvas(); }
  ...
  function openPop(pop, btnEl) {
    ...
    if (pop.classList.contains('hidden')) {
      ...
      const mr  = document.getElementById('map-wrap').getBoundingClientRect();
      const topFromMapBottom = mr.bottom - br.top + 8;
      pop.style.bottom = topFromMapBottom + 'px';
      pop.style.left   = Math.max(4, br.left - mr.left - 10) + 'px';
  ...
  document.getElementById('btnWind').addEventListener('click', function(e) {
    e.stopPropagation(); openPop(windPop, this);
  });
  document.getElementById('btnHum').addEventListener('click', function(e) {
    e.stopPropagation(); openPop(humPop, this);
  });
  ...
  /* Stop map clicks from closing popover */
  document.getElementById('map-wrap').addEventListener('click', e => {
    if (activePopover && !activePopover.contains(e.target) &&
        !document.getElementById('btnWind').contains(e.target) &&
        !document.getElementById('btnHum').contains(e.target)) {
      activePopover.classList.add('hidden');
      activePopover = null;
    }
  });
...
async function onMapClick(lat, lng) {
  ...
  try {
    ...
    console.error(err); renderError(err.message);
  } finally {
...
/* ══════════════════════════════════════════════════════
   12. WATER YIELD
══════════════════════════════════════════════════════ */
function estimateLWC(h){const Y=CFG.Y;if(h<Y.LWC_RH_MIN)return 0;let l;if(h>=Y.LWC_RH_HI)l=0.5+(h-Y.LWC_RH_HI)*0.10;else if(h>=Y.LWC_RH_MED)l=0.2+((h-Y.LWC_RH_MED)/(Y.LWC_RH_HI-Y.LWC_RH_MED))*0.30;else l=((h-Y.LWC_RH_MIN)/(Y.LWC_RH_MED-Y.LWC_RH_MIN))*0.20;return Math.min(l,Y.LWC_CAP);}
function estimateYield(humidity,windSpeed,elevation){
  ...
  const wf=Math.max(0,windSpeed-0.5);
  const daily=lwc*wf*Y.EFFICIENCY*Y.FOG_HOURS*3600/1000*ef;
...
function renderPanel({lat,lng,elevation,coastDist,weather,feasibility,waterYield}){
  ...
  document.getElementById('atmGrid').innerHTML=[
    {k:'HUMEDAD',v:weather.humidity,u:'%'},{k:'VIENTO',v:(weather.speed||0).toFixed(1),u:'m/s'},
    {k:'DIRECCIÓN',v:toDirCard(weather.dir||0),u:`${Math.round(weather.dir||0)}°`},{k:'TEMP',v:(weather.temp||0).toFixed(1),u:'°C'},
    {k:'NUBOSIDAD',v:weather.cloud||0,u:'%'},{k:'PRECIPIT.',v:(weather.precip||0).toFixed(1),u:'mm'},
...
function exportData(){
  ...
  return{latitud:p.lat,longitud:p.lon,elevacion_m:p.elevation,distancia_costa_km:S.coastDist??0,mes:S.selectedMonth,humedad_pct:w.humidity,viento_ms:w.speed||0,dir_viento_deg:w.dir||0,temperatura_c:w.temp||0,nubosidad_pct:w.cloud||0,precipitacion_mm:w.precip||0,lwc_g_m3:y?.lwc??0,rendimiento_l_m2_dia:y?.daily??0,eficiencia_colector:y?.efficiency??0,horas_niebla_dia:y?.fogHours??0,factibilidad_pct:f.overall,factibilidad_label:f.label,factor_niebla:f.factors[0].r,factor_elevacion:f.factors[1].r,factor_viento:f.factors[2].r,factor_costa:f.factors[3].r,factor_limitante:f.limiting?.name??'ninguno',timestamp:new Date().toISOString()};
}
