...
const CFG = {
  ...
  WIND: {
    PARTICLE_COUNT: 2200,
    FADE:       0.96,        // slightly longer trail (less fade per frame)
    SPEED_SCALE: 0.13,       // ← KEY CHANGE: was 0.38, much slower
    LINE_WIDTH:  1.8,
   MAX_AGE:     160,        // longer life since they move slower
   MAX_ALPHA:   0.82,
   MIN_ALPHA:   0.32,
},
  F: {
...
function buildUVGrid(samples, sw, ne, cols, rows) {
  ...
  for(let r=0;r<rows;r++) for(let c=0;c<cols;c++) {
    ...
    uv[idx]   = -speed * Math.sin(rad);   // u: east+  for westerly (270°) ✓
    uv[idx+1] =  speed * Math.cos(rad);   // v: +south, used as p.y += v below
  }
  return uv;
}
